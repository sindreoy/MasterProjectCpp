//
// Created by Sindre Bakke Øyen on 25.04.2018.
//
...
int main() {
    ...
    realtype k1lb = log10(1.e-3), k1ub = log10(5.e-3);
    realtype k2lb = log10(500), k2ub = log10(1000);
    size_t i, j, q, r;
    for (i = 0; i < Npts; i++){
    ...
    for (i = 0; i < Npts; i++){
        ...
        for (j = 0; j < Npts; j++){
            ...
            M = m[index_ij]->getM();
            N = m[index_ij]->getN();
            residualVector[index_ij] = gsl_matrix_alloc(M, N);
            for (q = 0; q < M; q++) {
                for (r = 0; r < N; r++) {
                    gsl_matrix_set(
                            residualVector[index_ij], q, r,
                            m[index_ij]->getResidualij(q, r)
                    );
                }
            }
            delete m[index_ij];
...
//    std::ofstream fparams("../results/paramsBreakage.dat");
    ...
    std::ofstream fresiduals("../results/residualFiles/residualsDynamics" + str + ".dat");
